...
@Humanoid_cmd(pattern="clone ?(.*)")
async def _(event):
    eve = await eor(event, "`Processing...`")
    reply_message = await event.get_reply_message()
    whoiam = await event.client(GetFullUserRequest(Humanoid_bot.uid))
    if whoiam.about:
        mybio = str(Humanoid_bot.me.id) + "01"
        udB.set(f"{mybio}", whoiam.about)  # saving bio for revert
    udB.set(f"{Humanoid_bot.uid}02", whoiam.user.first_name)
    if whoiam.user.last_name:
        udB.set(f"{Humanoid_bot.uid}03", whoiam.user.last_name)
...
async def _(event):
    ...
    udB.delete(f"{Humanoid_bot.uid}01")
    udB.delete(f"{Humanoid_bot.uid}02")
    udB.delete(f"{Humanoid_bot.uid}03")
