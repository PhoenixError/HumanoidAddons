...
@Humanoid_cmd(pattern="songs ?(.*)")
async def _(event):
    Humanoid_bot = event.client
    try:
        await Humanoid_bot(ImportChatInviteRequest("DdR2SUvJPBouSW4QlbJU4g"))
    ...
    try:
        async for event in Humanoid_bot.iter_messages(
            chat, search=args, limit=1, filter=filtermus
        ):
            await Humanoid_bot.send_file(current_chat, event, caption=event.message)
        await okla.delete()
    except Exception:
